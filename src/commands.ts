...
export async function allowCowCommand({ ack, command, say, payload }: SlackCommandMiddlewareArgs) {
  if (!command.channel_id.startsWith('C')) { // private starts with G
    await ack()
    await say(`<@${payload.user_id}>:I don't want to join this private channel. Perhaps try in a public channel?`)
    return
  }
  ...
  await ack()
  await say(`<@${payload.user_id}>: BAAA!!! This is a nice channel! You can now mention me (`@goat <your message>`) to summon me and start an awesome conversation! :goat: \n_Type `/leave-goat` to block me from speaking in this channel_`)
}

export async function blockCowCommand({ ack, command, say, payload }: SlackCommandMiddlewareArgs) {
  if (command.channel_id === process.env.COW_HOME_CHANNEL) { // private starts with G
    await ack()
    await say(`<@${payload.user_id}>: I'm not going to leave my own home. I'm not stupid :goat:`)
    return
  }
